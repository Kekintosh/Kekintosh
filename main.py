...
WORLD_GENERATION_STARTED = False
...
def quitToMenu():
    ...
    for x in range(0, scene.WIDTH, tex.width):
        for y in range(0, scene.HEIGHT, tex.height):
            tex.blit(x, y)
            tex2.blit(x, y)
    drawInfoLabel(scene, "Išeinama į pagrindį meniu", xx=scene.WIDTH // 2, yy=scene.HEIGHT // 2,
...
def drawDeathScreen(mc):
    ...
    drawInfoLabel(scene, f"Tu mirei!", xx=scene.WIDTH // 2, yy=scene.HEIGHT - scene.HEIGHT // 4, style=[('', '')],
...
def pauseMenu(mc):
    ...
    drawInfoLabel(scene, f"Žaidimo meniu", xx=scene.WIDTH // 2, yy=scene.HEIGHT - scene.HEIGHT // 4, style=[('', '')],
...
def drawMainMenu(mc):
    ...
    drawInfoLabel(scene, f"TechnikCraft {MC_VERSION}", xx=10, yy=10, style=[('', '')], size=12)
...
pygame.display.set_caption(f"TechnikCraft {MC_VERSION}")
...
gui.GUI_TEXTURES = {
    ...
    "chest_window": pyglet.resource.image("gui/chest_window.png"),
    "furnace_window": pyglet.resource.image("gui/furnace_window.png"),
    "lit_progress": pyglet.resource.image("gui/lit_progress.png")
}
...
texture = gui.GUI_TEXTURES["lit_progress"]
texture.width *= 2
texture.height *= 2

texture = gui.GUI_TEXTURES["furnace_window"]
texture.width *= 2
texture.height *= 2

texture = gui.GUI_TEXTURES["chest_window"]
texture.width *= 2
texture.height *= 2
...
try:
    ...
    with open("gui/splashes.txt", "r", encoding='utf-8') as splfile:
        splash = splfile.read().split("\n")
        splash = splash[randint(0, len(splash) - 1)]
except UnicodeDecodeError:
    # Fallback to system encoding if UTF-8 fails
    with open("gui/splashes.txt", "r", encoding='latin1') as splfile:
        splash = splfile.read().split("\n")
        splash = splash[randint(0, len(splash) - 1)]
except FileNotFoundError:
    print("Splashes file not found! Using default splash.")
    splash = "Default splash text" 

sound.musicPlayer.play()
sound.musicPlayer.set_volume(sound.volume)

# Main menu buttons
singleplayerButton = Button(scene, "Žaisti", 0, 0)
optionsButton = Button(scene, "Nustatymai", 0, 0)
quitButton = Button(scene, "Išeiti iš žaidimo", 0, 0)
...
# Pause menu buttons
resumeButton = Button(scene, "Grįžti žaisti", 0, 0)
            ...
            if scene.allowEvents["keyboardAndMouse"]:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.button == 1:
                        ...
                        player.inventory.viewmodel.start_mining()
                    elif event.button == 3:
                        player.inventory.attack_single()
                elif event.type == pygame.MOUSEBUTTONUP:
                    if event.button == 1:
                        left_mouse_held = False
                        player.inventory.viewmodel.stop_mining()
                    if event.button == 4:
                        player.inventory.activeInventory -= 1
